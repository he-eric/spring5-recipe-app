...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Optional<UnitOfMeasure>> getUnitOfMeasures(List<String> unitOfMeasures) {
        List<Optional<UnitOfMeasure>> unitOfMeasureOptionals = new ArrayList<>();
        Iterator it = unitOfMeasures.iterator();
        while (it.hasNext()) {
            Optional<UnitOfMeasure> optional = unitOfMeasureRepository.findByDescription((String)it.next());
            if (!optional.isPresent()) {
                throw new RuntimeException("Expected UOM Not Found");
            }
            unitOfMeasureOptionals.add(optional);
        }
        return unitOfMeasureOptionals;
    }

    private List<Recipe> getRecipes() {

        List<Recipe> recipes = new ArrayList<>(2);
        List<String> unitOfMeasures = Arrays.asList("Each", "Tablespoon", "Teaspoon", "Dash", "Pint", "Cup");
        List<Optional<UnitOfMeasure>> uomOptionals = getUnitOfMeasures(unitOfMeasures);

        //get optionals
        UnitOfMeasure eachUom = uomOptionals.get(0).get();
        UnitOfMeasure tableSpoonUom = uomOptionals.get(1).get();
        UnitOfMeasure teapoonUom = uomOptionals.get(2).get();
        UnitOfMeasure dashUom = uomOptionals.get(3).get();
        UnitOfMeasure pintUom = uomOptionals.get(4).get();
        UnitOfMeasure cupsUom = uomOptionals.get(5).get();
